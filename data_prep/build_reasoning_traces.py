...
data = pd.DataFrame(pd.read_json("clef_2026_checkthat_english_train.json"))
...
# +
data["num_traces"] = data["Reasoning_traces"].apply(len)

# Frequency table
print(data["num_traces"].value_counts().sort_index())

# Summary stats
print(data["num_traces"].describe())

# +
from collections import Counter

# Flatten all verdicts across all claims
all_verdicts = [v.lower() for verdicts in data["Verdict_list"] for v in verdicts]

# Frequency count
verdict_freq = Counter(all_verdicts)
print(verdict_freq)
# -
